...
export const useChatBotStore = create<ChatBotState>((set, get) => ({
  ...
  getContextualResponses: (message: string, userType: string) => {
    ...
    if (message.includes('exposant') || message.includes('entreprise')) {
      return {
        content: `🏢 SIPORTS 2026 accueille ${siportsKnowledge.salon.exposants}+ exposants internationaux répartis dans 5 pavillons thématiques. Voulez-vous explorer un pavillon spécifique ?`,
        type: 'suggestion',
        suggestions: [
    ...
    if (message.includes('événement') || message.includes('conférence') || message.includes('programme')) {
      return {
        content: `📅 Le programme SIPORTS comprend ${siportsKnowledge.salon.conferences}+ conférences, ateliers et sessions de networking. Quel type d'événement vous intéresse ?`,
        type: 'quick_reply',
    ...
    if (message.includes('rendez-vous') || message.includes('rdv')) {
      const rdvText = userType === 'visitor' 
        ? "Vous pouvez programmer jusqu'à 5 RDV B2B garantis avec les exposants"
