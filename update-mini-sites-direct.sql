DO $$ 
BEGIN
  -- Mise à jour directe des mini-sites avec les nouvelles sections
  -- en ajoutant les sections manquantes (certifications, gallery, testimonials)
  -- tout en évitant l'erreur du trigger avec updated_at vs last_updated
  
  -- Récupérer tous les mini-sites
  FOR minisite IN
    SELECT id, sections FROM mini_sites LIMIT 5
  LOOP
    -- Variable pour les sections mises à jour
    DECLARE updated_sections jsonb;
    DECLARE has_certifications boolean := false;
    DECLARE has_gallery boolean := false;
    DECLARE has_testimonials boolean := false;
    
    -- Vérifier quelles sections existent déjà
    IF minisite.sections IS NOT NULL THEN
      IF EXISTS (
        SELECT 1 FROM jsonb_array_elements(minisite.sections) AS section
    ...
    -- Initialiser avec les sections existantes ou un tableau vide
    updated_sections := COALESCE(minisite.sections, '[]'::jsonb);
    
    -- Ajouter certification si manquante
    IF NOT has_certifications THEN
      updated_sections := updated_sections || jsonb_build_array(
        jsonb_build_object(
          'type', 'certifications',
    ...
    -- Ajouter gallery si manquante
    IF NOT has_gallery THEN
      updated_sections := updated_sections || jsonb_build_array(
        jsonb_build_object(
          'type', 'gallery',
          'content', jsonb_build_object(
    ...
    -- Ajouter testimonials si manquante
    IF NOT has_testimonials THEN
      updated_sections := updated_sections || jsonb_build_array(
        jsonb_build_object(
          'type', 'testimonials',
          'content', jsonb_build_object(
    ...
    -- Mettre à jour le mini-site en contournant le trigger
    UPDATE mini_sites
    SET sections = updated_sections,
        last_updated = NOW()
    WHERE id = minisite.id;
    
    RAISE NOTICE 'Mini-site % mis à jour', minisite.id;
  END LOOP;
  
  RAISE NOTICE 'Mise à jour des mini-sites terminée';
END $$;
